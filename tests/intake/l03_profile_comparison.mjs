...
import {compare,configuration,hostGate,loadOriginal,verdict,CASES,BUDGET,privilegedCommand} from '../../ci/l03_profile_comparison.mjs';
...
const facts={platform:'linux',provider:'github-hosted',imageOS:'ubuntu24',imageVersion:'measured-image',event:'pull_request',attempt:'1',repository:'Farobertogu/ledgerdesk',branch:'card/INC-03-l03-single-profile-comparison',workflow:'l03-profile-comparison',commit:'a'.repeat(40),checkout:'a'.repeat(40),vm:'microsoft',socket:'/run/docker.sock',socketType:'socket',rootless:false,cgroupVersion:'2',driver:'systemd',containers:[]};
...
test('PC-01 normal counterpart: one primary per slot, unchanged 14-slot order and restoration',async()=>{
 const m=model(),r=await compare(m.ports);assert.equal(r.status,'no_discrimination');assert.equal(r.rows.length,14);assert.equal(m.runs,14);assert.equal(m.restarts,15);assert.equal(r.restoration,'confirmed');assert.equal(m.current,originalText);
 ...
 assert.deepEqual(r.rows.map(v=>v.id),['A-1','B-1','B-2','A-2','A-3','B-3','B-4','A-4','A-5','B-5','B-6','A-6','A-7','B-7']);
 assert.equal(m.trace.filter(v=>v==='validate').length,14);assert.equal(m.trace.filter(v=>v==='install').length,14);
...
for(const mode of ['primary-error-cleanup-ok','primary-error-cleanup-error','returned-row-cleanup-error'])test('PC-01 exact outcome retention: '+mode,async()=>{
 ...
 assert.equal(m.trace.filter(v=>v==='primary').length,1);assert.equal(r.switches,1);assert.equal(r.status,'setup_or_measurement_failure');
 ...
 assert.equal(r.restoration,failedCleanup?'blocked_unexpected_resources':'confirmed');assert.equal(m.trace.includes('restore'),!failedCleanup);assert.equal(m.trace.includes('verify-restored'),!failedCleanup);
...
test('PC-01 write failure is reported without erasing outcomes or claiming guaranteed durable evidence',async()=>{
 ...
 assert.equal(r.restoration,'blocked_unexpected_resources');assert(!m.records.some(v=>v.name==='A-1-failure'));assert.deepEqual(m.records.find(v=>v.name==='comparison-result').value,r);assert(!JSON.stringify(m.records).includes('do-not-export'));
...
for(const [mode,status,phase,code]of [
 ...
 assert.deepEqual(observed,{status,errors:code?[code]:[],caseFailures:code?[{caseId:'B-7',phase,code}]:[],rows:14,primaries:['A-1','B-1','B-2','A-2','A-3','B-3','B-4','A-4','A-5','B-5','B-6','A-6','A-7','B-7'],restarts:15,restores:1,verified:1,restoration:'confirmed',caseRecord:mode!=='last-case-write-error',finalStatus:status,maximumWriteAttempts:1});
...
for(const [fault,status,runs,restoration]of [
 ['initial-stopped','setup_or_measurement_failure',0,'not_needed'],['invalid-config','setup_or_measurement_failure',0,'not_needed'],['readiness','setup_or_measurement_failure',0,'confirmed'],['measurement','setup_or_measurement_failure',1,'confirmed'],['B-fail','candidate_unqualified',3,'confirmed'],['control-fail','candidate_unqualified',1,'confirmed'],['restore','setup_or_measurement_failure',14,'unconfirmed'],['cleanup','setup_or_measurement_failure',1,'blocked_unexpected_resources']
...
test('Runtime route has no automatic pair, old branch gate or hidden memory calibration',()=>{
 ...
 assert(!host.includes('runKernelControl'));assert(!source.includes('kernelIdleControl'));assert(!workflow.includes('workflow_dispatch'));assert(workflow.includes('types: [opened]'));assert(workflow.includes('runs-on: ubuntu-24.04'));assert(!workflow.includes('continue-on-error'));
