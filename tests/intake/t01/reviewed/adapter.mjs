...
export function strictText(bytes, mutation) {
  if (mutation === 'replace-invalid-utf8') return { text: bytes.toString('utf8'), bomBytes: 0 };
...
export async function extract(inputPath, profile, mutation) {
  if ((await fs.stat(inputPath)).size > PROFILE.limits.inputBytes) fail('input_limit');
  const bytes = await fs.readFile(inputPath);
  if (bytes.length > PROFILE.limits.inputBytes) fail('input_limit');
  const original = { name: path.basename(inputPath), bytes: bytes.length, sha256: digest(bytes) };
  let raw;
  let elements = [];
  let inventory;
  let unsupported = [];
  if (profile === 'text' || profile === 'csv') {
    const decoded = strictText(bytes, mutation);
    if (profile === 'text') {
      raw = decoded;
      ...
      const options = { ...PROFILE.csv, ...(mutation === 'collapse-csv-headers' ? { columns: true } : {}) };
      ...
      const rows = raw.map((record, i) => {
        ...
        if (mutation !== 'omit-explicit-record-bound' && values.reduce((sum, value) => sum + value.length, 0) > PROFILE.limits.csvRecordUnits) fail('csv_record_limit');
    ...
    for (const [index, sheet] of sheets.entries()) {
      ...
      for (const row of rows) for (const cell of array(row.c)) {
        ...
        let formulaValue = formula ? { storedExpression: scalar(cell.f), attributes: typeof cell.f === 'object' ? Object.fromEntries(Object.entries(cell.f).filter(([k]) => k.startsWith('@_'))) : {}, cached: { availability: stored ? 'observed' : 'not_available', ...(stored ? { lexical: scalar(cell.v), sourceType: cell['@_t'] ?? 'n' } : {}) } } : undefined;
        if (mutation === 'recalculate-cache' && address === 'D2' && formulaValue) formulaValue.cached = { availability: 'observed', lexical: '25.00', sourceType: 'n' };
        if (mutation === 'missing-cache-is-zero' && formulaValue?.cached.availability === 'not_available') formulaValue.cached = { availability: 'observed', lexical: '0', sourceType: 'n' };
