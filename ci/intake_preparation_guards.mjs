...
// controls mutate captured copies; a timeout is never a killed mutant.
export const preparationGuards=Object.freeze([
  ...
  {group:'identity',name:'hash-identity',tests:['C03 possible duplicate']},
  {group:'disclosure',name:'hidden-duplicate-status',tests:['DISC01']},
  {group:'ui-preparation',wholeJourney:true,sourceCount:2,name:'auto-publish-candidate',tests:['W10'],
    sources:['src/server/intake/preparation/proposals.ts','src/server/intake/postgres/008_preparation_data.sql'],
    assertion:["expected: 'candidate'","actual: 'published'"]},
]);

if(process.argv[1]&&path.resolve(process.argv[1])===fileURLToPath(import.meta.url)){
  const flags=process.argv.slice(2);let cases=preparationGuards;
  if(flags.length){assert.equal(flags.length,2);assert.equal(flags[0],'--faults');const names=flags[1].split(',');
    assert.ok(names.length&&new Set(names).size===names.length&&names.every(n=>cases.some(c=>c.name===n)),'Known unique preparation faults required');
  ...
  for(const c of cases){if(!seen.has(c.group)){plan.push({group:c.group,name:null,wholeJourney:c.wholeJourney});seen.add(c.group);}plan.push(c);}
  try{for(const c of plan){
    const args=['--experimental-strip-types','ci/intake_t02_check.mjs','--group','extraction','--extraction-cases','preparation',
      '--preparation-cases',c.group,...(c.wholeJourney?['--whole-journey']:[]),...(c.name?['--preparation-mutation',c.name]:[])];
