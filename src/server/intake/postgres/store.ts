...
export class IntakeStore extends AccessStore {
  ...
  async reacquireAdmission(exclusive:boolean){
    await super.reacquireAdmission(exclusive);
    this.observedPid=(await this.client.query('SELECT pg_backend_pid() AS pid')).rows[0].pid;
    this.observation?.({origin:'admission-boundary',kind:'reacquired',backendPid:this.observedPid,atMs:Date.now()});
  }
  ...
  async close(){this.closing??=super.close();return this.closing;}
}
