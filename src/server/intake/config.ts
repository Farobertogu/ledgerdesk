...
export type IntakeConfig = Readonly<{
  ...
  preparation?: 'intake-preparation/1';
}>;
...
export function intakeConfig(input: IntakeConfig): IntakeConfig {
  if (!input || Object.keys(input).filter(key=>key!=='extraction'&&key!=='preparation').sort().join(',') !==
      ...
      (Object.hasOwn(input,'preparation')&&(input.preparation!=='intake-preparation/1'||input.extraction!=='intake-execution/1')) ||
