...
|---|---|---|---|
| I22 | `adr_gate` hardening: normalised gate with a case that turns red on a missing ADR | I0b | the red case reproduced on Windows and in CI + `check` green |
| I19 | Act-1 stage lines: the `retrieval.ts` amendment naming the absent/not-retrieved pair + the on-screen half of the provenance sentence under the citations; the opening line is spoken, never painted | I7 | the two on-product texts on screen + the spoken lines carried by the demo script + `ci/check.sh` green |
